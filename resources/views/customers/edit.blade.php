...
<div class="form-group">
    <label for="unknown_product_code">
        Unknown Product Code - when verified remove this field
    </label>
...
@endif
        ...
        <div class="form-group">
            ...
            <input type="text" id="phone_number" name="phone_number" value="{{ $customer->phone_number }}" required placeholder="e.g. +94..." maxlength="20">
        </div>

        <div class="form-group">
            <label for="phone_number_2"><i class="fas fa-phone"></i> Phone Number 2</label>
            <input type="text" id="phone_number_2" name="phone_number_2" value="{{ $customer->phone_number_2 ?? '' }}" placeholder="Secondary phone number" maxlength="20">
        </div>
        ...
        <div class="form-group">
            ...
            <input list="cityList" id="city" name="city" class="form-control"
                value="{{ $customer->city }}" placeholder="Type or select city">
        ...
        <div class="form-group">
            ...
            <input type="text" id="district" name="district" class="form-control" value="{{ $customer->district }}">
        </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Known city -> district & province map
    const cityMap = {
    ...
    // District -> province map
    const districtMap = {};
    Object.values(cityMap).forEach(([district, province]) => {
        districtMap[district.toLowerCase()] = province;
    });
    ...
    function updateFromCity(){
        ...
        if(cityMap[cityKey]){
            ...
            districtInput.value = '';
            provinceInput.value = '';
            districtInput.readOnly = false; // allow typing new district
        }
    }

    function updateFromDistrict(){
        const districtKey = normalize(districtInput.value);
        if(districtMap[districtKey]){
            provinceInput.value = districtMap[districtKey];
        } else {
            provinceInput.value = '';
        }
    }
    ...
    districtInput.addEventListener('input', updateFromDistrict);
    districtInput.addEventListener('change', updateFromDistrict);
